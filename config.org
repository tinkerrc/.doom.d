...
(setq doom-unicode-font (font-spec :family "Hiragino Sans GB" :size 18))
...
(use-package! org
  ...
  (setq cdlatex-math-modify-alist '((?\" "\\mathbb" nil t nil nil)
                                    (?t "\\text" nil t nil nil)))
  <<org-hooks>>
      ...
      '(("d" "Daily Dashboard"
         ((agenda "a" ((org-agenda-span 'day)
                       ...
                       (org-agenda-sorting-strategy
                        '(time-up priority-down todo-state-up category-keep))
        ...
        ("D" "Upcoming deadlines" agenda ""
         ((org-agenda-archives-mode nil)
          ...
          (org-agenda-sorting-strategy
           '(deadline-up todo-state-up priority-down category-keep))
...
(map! :map org-roam-mode-map
      ...
      (:prefix ("r" . "org-roam")
       ...
       (:prefix ("d" . "org-roam-dailies")
        :desc "Find Yesterday"                   :ni  "y"        #'org-roam-dailies-goto-yesterday
        :desc "Find Today"                       :ni  "j"        #'org-roam-dailies-goto-today
        :desc "Find Tomorrow"                    :ni  "k"        #'org-roam-dailies-goto-tomorrow
        :desc "Find Date"                        :ni  "d"        #'org-roam-dailies-goto-date
       ...
       :desc "Add tag"                          :ni  "t"      #'org-roam-tag-add
       :desc "Delete tag"                       :ni  "T"      #'org-roam-tag-delete
       :desc "Add alias"                        :ni  "a"      #'org-roam-alias-add
       :desc "Delete alias"                     :ni  "A"      #'org-roam-alias-delete
...
(setq org-roam-capture-templates
      `(,(make-roam-template "Default" :tags "" :key "d")
        ,(make-roam-template "Concept" :after "* Definition\n%?" :setup "./setup-concept.org")
      ...
      `(("j" "Journal" entry "* %?"
         :target (file+head+olp "%<%Y-%m-%d>.org" ,(get-org-template "org-roam-daily-head") ("Journal")))

        ("b" "Braindump" entry "* Braindump %<%Y-%m-%d %H:%M>\n- %?"
         :target (file+head "%<%Y-%m-%d>.org" ,(get-org-template "org-roam-daily-head")))

        ("p" "Plan" item "- %?"
         :target (file+head+olp "%<%Y-%m-%d>.org" ,(get-org-template "org-roam-daily-head") ("Daily Plan")))

        ("l" "Log" item "- (%<%H:%M>) %?"
         :target (file+head+olp "%<%Y-%m-%d>.org" ,(get-org-template "org-roam-daily-head") ("Logs")))

        ("W" "Weekly Review" entry ,(concat-nl "* Weekly Review for %(format-time-string \"Week %V of %Y\") (%(org-current-week-range))"
                     "- Braindump"
                     "  What's on your mind?"
                     "  - %?"
                     "- Checklist"
                     "  - [ ] Review agenda; (re)schedule & prune tasks"
        ...
        ("M" "Monthly Review" entry "* Monthly Review for %(format-time-string \"%B of %Y\")\n%?"
         :target (file+head "%<%Y-%m>.org" ,(get-org-template "org-roam-daily-head")))

        ("Y" "Yearly Review" entry "* Yearly Review for %(format-time-string \"%Y\")\n%?"
         :target (file+head "%<%Y>.org" ,(get-org-template "org-roam-daily-head")))))
#+end_src
