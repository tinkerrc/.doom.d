...
#+name: org-options
#+begin_src emacs-lisp :tangle no
(setq-default org-download-image-dir (org-file "static/"))
(setq org-attach-preferred-new-method 'dir
      org-attach-directory (org-file "static/")
...
(map! :map org-mode-map
      ...
      :localleader
      :desc "Start Depend Chain"          :ni "-"       (cmd! (save-excursion
                                                                ...
                                                                (if (not (org-goto-first-child))
                                                                         (user-error "Project has no child task"))
                                                                (let ((last-point (point)))
                                                                  (while (and (or (string= (nth 2 (org-heading-components)) "DONE")
                                                                                  (string= (nth 2 (org-heading-components)) "KILL"))
                                                                              (outline-get-next-sibling))
                                                                    (setq last-point (point)))
                                                                  (goto-char last-point))
                                                                  (if (or (eq nil (nth 2 (org-heading-components)))
                                                                          (not (org-at-heading-p)))
                                                                      (user-error "No valid candidate for start-of-chain found"))
                                                                  (if (string= (nth 2 (org-heading-components)) "TODO")
                                                                      (org-todo "NEXT"))
                                                                  (org-set-property "TRIGGER" "chain-find-next(NEXT)")))

      :desc "Extract Org link"            :ni "ly"      #'org-extract-link
      :desc "Copy Org Id"                 :ni "li"      (cmd! (kill-new (org-id-get nil t)))
      :desc "Toggle Timestamp Range Type" :ni "x"       (cmd! (save-excursion
...
(map! :map org-roam-mode-map
      ...
      (:prefix ("r" . "org-roam")
       ...
       :desc "Find Node No-select"              :ni  "?"        #'org-roam-node-find-noselect
...
(setq org-roam-capture-templates
      `(,(make-roam-template "Topic" :after "* ${title}\n%?")
        ,(make-roam-template "Concept" :after "* Definition: ${title}\n%?" :setup "./setup-concept.org")
        ,(make-roam-template "Blog Post" :tags "blog")
        ,(make-roam-template "Person" :after "* Biography: ${title}\n%?")
        ,(make-roam-template "Custom" :tags "" :key "u")
        ,(make-roam-template "Project" :key "P" :fn "proj-${slug}" :cats '("Project"))
        ("r" "ref" plain "%?"
...
(cl-defun make-roam-template
  ...
  (unless head
    (setq head
          (concat "#+title: ${title}\n"
                  "#+roam_alias: \n"
                  (when (string-prefix-p "Ref" desc)
                    "#+roam_key: ${ref}\n")
                  (when (string= desc "Post")
                    "#+author: Zhenkai Weng\n"
                    "#+date: %<%Y-%m-%d>\n")
                  "#+roam_tags: " tags "\n"
                  (when (string= desc "Post")
...
(setq confirm-kill-processes nil
      ...
      projectile-project-search-path '("~/re/dev")
